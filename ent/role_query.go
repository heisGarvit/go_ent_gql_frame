...
type RoleQuery struct {
	...
	withNamedPermissions map[string]*RolePermissionQuery
...
func (rq *RoleQuery) QueryCompany() *CompanyQuery {
...
func (rq *RoleQuery) WithCompany(opts ...func(*CompanyQuery)) *RoleQuery {
	query := (&CompanyClient{config: rq.config}).Query()
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withCompany; query != nil {
		if err := rq.loadCompany(ctx, query, nodes, nil,
			func(n *Role, e *Company) { n.Edges.Company = e }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range rq.withNamedPermissions {
		if err := rq.loadPermissions(ctx, query, nodes,
			...
			func(n *Role, e *RolePermission) { n.appendNamedPermissions(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *RoleQuery) loadCompany(ctx context.Context, query *CompanyQuery, nodes []*Role, init func(*Role), assign func(*Role, *Company)) error {
...
func (rq *RoleQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := rq.ctx.Fields; len(fields) > 0 {
		...
		if rq.withCompany != nil {
			_spec.Node.AddColumnOnce(role.FieldCompanyID)
		}
	}
...
func (rq *RoleQuery) WithNamedPermissions(name string, opts ...func(*RolePermissionQuery)) *RoleQuery {
	query := (&RolePermissionClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedPermissions == nil {
		rq.withNamedPermissions = make(map[string]*RolePermissionQuery)
	}
