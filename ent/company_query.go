...
func (cq *CompanyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Company, error) {
	...
	for name, query := range cq.withNamedRoles {
		if err := cq.loadRoles(ctx, query, nodes,
			func(n *Company) { n.appendNamedRoles(name) },
			func(n *Company, e *Role) { n.appendNamedRoles(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CompanyQuery) loadRoles(ctx context.Context, query *RoleQuery, nodes []*Company, init func(*Company), assign func(*Company, *Role)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(role.FieldCompanyID)
	}
	query.Where(predicate.Role(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(company.RolesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (cq *CompanyQuery) WithNamedRoles(name string, opts ...func(*RoleQuery)) *CompanyQuery {
	query := (&RoleClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if cq.withNamedRoles == nil {
		cq.withNamedRoles = make(map[string]*RoleQuery)
	}
	cq.withNamedRoles[name] = query
	return cq
}
