...
CREATE TABLE "companies" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "created_at" timestamptz NOT NULL DEFAULT now(), "updated_at" timestamptz NOT NULL DEFAULT now(), "company_id" uuid NULL, "name" character varying NOT NULL, "is_disabled" boolean NOT NULL DEFAULT false, "owner_id" uuid NULL, PRIMARY KEY ("id"));
...
-- Create "users" table
CREATE TABLE "users" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "created_at" timestamptz NOT NULL DEFAULT now(), "updated_at" timestamptz NOT NULL DEFAULT now(), "name" character varying NULL, "email" character varying NOT NULL, "mobile" character varying NOT NULL, "password" character varying NOT NULL, "is_disabled" boolean NOT NULL DEFAULT false, "date_of_birth" date NULL, "date_of_join" date NOT NULL, "timing_start" time NULL, "employee_id" character varying NULL, "role_id" uuid NULL, "company_id" uuid NULL, PRIMARY KEY ("id"), CONSTRAINT "users_companies_company" FOREIGN KEY ("company_id") REFERENCES "companies" ("id") ON UPDATE NO ACTION ON DELETE SET NULL);
